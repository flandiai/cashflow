...
export const AssetManagement = ({ player, updatePlayerState, showAlertMessage, assets, setAssets }) => {
    ...
    return (
        <Tabs defaultValue="realestate" className="w-full">
            ...
            <TabsContent value="stocks">
                ...
                <Button onClick={handleBuyStock} className="mt-4">Aktien kaufen</Button>
            </TabsContent>
            <TabsContent value="gold">
                ...
                <Input
                    ...
                    className="mt-2"
                />
                <Button onClick={handleBuyGold} className="mt-4">Gold kaufen</Button>
            </TabsContent>
        </Tabs>
    );
};

export const ViewAssets = ({ assets, setAssets, updatePlayerState, showAlertMessage }) => {
    const [sellPrice, setSellPrice] = useState({});
    const [sellQuantity, setSellQuantity] = useState({});

    const handleSellAsset = (asset, index) => {
        ...
        if (asset.type === "Immobilie" || quantity === asset.quantity) {
            ...
            setAssets(assets.map((a, i) => 
                i === index ? {...a, quantity: a.quantity - quantity} : a
            ));
        }

        showAlertMessage(`Sie haben ${asset.type === "Immobilie" ? asset.name : quantity + " " + asset.name} für insgesamt ${totalSalePrice}€ verkauft.`);
    };
            ...
            {assets.map((asset, index) => (
                <div key={index} className="mb-4 p-4 border rounded">
                    ...
                    <Input
                        ...
                        className="mt-2"
                    />
                    <Button onClick={() => handleSellAsset(asset, index)} className="mt-2">Verkaufen</Button>
                </div>
            ))}
        </div>
    );
