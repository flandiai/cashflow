...
const CashFlowApp: React.FC = () => {
  ...
  const calculateTotalValues = useCallback(() => {
    const totalPassiveIncome = assets.reduce(
      (total, asset: Asset) => total + (asset.cashflow || 0),
      0,
    );
    setPlayer((prev) => ({
      ...prev,
  ...
  const handleChange = (field: keyof typeof player, value: string) => {
    const newValue = Math.max(0, parseInt(value) || 0);
    setPlayer((prev) => ({ ...prev, [field]: newValue }));
  };

  const adjustValue = (field: keyof typeof player | 'tempExpenseIncome', amount: number) => {
    if (field === 'tempExpenseIncome') {
      setTempExpenseIncome((prev) => prev + amount);
    } else {
      handleChange(field, (player[field] + amount).toString());
    }
  };
  ...
  const handleLoan = (action: 'add' | 'repay') => {
    if (action === 'add') {
      ...
      showAlertMessage(
        'Sie haben einen Kredit über 1000€ aufgenommen. Ihre monatlichen Ausgaben sind um 100€ gestiegen.',
      );
  ...
  const handleExpenseIncomeSubmit = () => {
    ...
    showAlertMessage(
      `${tempExpenseIncome >= 0 ? 'Einnahme' : 'Ausgabe'} von ${Math.abs(tempExpenseIncome)}€ gebucht.`,
    );
  };

  const renderInputWithAdjust = (
    field: keyof typeof player | 'tempExpenseIncome',
    label: string,
  ) => (
    <div className="flex items-center mb-2">
      ...
      <Input
        ...
        onChange={(e) =>
          field === 'tempExpenseIncome'
            ? setTempExpenseIncome(parseInt(e.target.value) || 0)
            : handleChange(field, e.target.value)
        }
        className="w-1/3 mr-2 bg-gray-800 text-white"
      />
      <Button onClick={() => adjustValue(field, -100)} className="mr-1 bg-red-500 text-white">
        <Minus size={16} />
      </Button>
      <Button onClick={() => adjustValue(field, 100)} className="bg-green-500 text-white">
        <Plus size={16} />
      </Button>
    </div>
  );
  ...
  return (
    <div className="p-4 bg-gray-800 text-white min-h-screen relative">
        ...
        <Card className="bg-gray-700 text-white">
          ...
          <CardContent>
            ...
            <p className="text-red-500">
              Fehlendes passives Einkommen: {Math.max(0, player.expenses - player.passive)}€
            </p>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent>
          ...
          <div className="flex items-center mt-2">
            ...
            <span className="bg-red-500 text-white px-2 py-1 rounded">
              {currentLoan > 0 ? Math.floor(currentLoan / 10) : 0}€
            </span>
          </div>
          <div className="flex items-center mt-2">
            ...
            <span
              className={`px-2 py-1 rounded ${
                player.salary + player.passive - player.expenses >= 0
                  ? 'bg-green-500'
                  : 'bg-red-500'
              }`}
            >
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
        <Button onClick={handlePayday} className="bg-green-500 text-white">
          Zahltag
        </Button>
        <Button onClick={handleBuyAsset} className="bg-purple-500 text-white">
          Asset kaufen
        </Button>
